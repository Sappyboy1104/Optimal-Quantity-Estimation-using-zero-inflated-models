...
@st.cache_data
def load_data(nrows):
    df = pd.read_excel("Trichy_Analysis_New.xlsx")
    df = df.iloc[:nrows,1:]
    df = df[df["Status"]=="Success"]
    return df

st.header("Demand Data Analysis of Plant 1")
...
st.header("Goodness of fit and Relative Model Comparision using p value")
gof_plant_1 = [
    {
        "Model":"ZISPD",
        "% SKU showing Strong Fit": "99.5%"
    },
    {
        "Model":"ZINB",
        "% SKU showing Strong Fit": "99.4%"
    },
    {
        "Model":"ZIP",
        "% SKU showing Strong Fit": "96.4%"
    },
    {
        "Model":"ZIGP",
        "% SKU showing Strong Fit": "70.42%%"
    },
    {
        "Model":"Poisson",
        "% SKU showing Strong Fit": "65.04%"
    }

]

df = pd.DataFrame(gof_plant_1)
